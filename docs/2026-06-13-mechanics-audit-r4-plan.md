...
- **[C6] 关键物品主动**(进行中):✅ Shiva 被动减速光环(holderModifier aura,-30% 攻速 750 内)、✅ Heart 脱战 6 秒后 2%/s 回血(holderModifier onTick)——**附带修复 syncHolderModifiers 未初始化 nextTickAt 导致持有型 onTick 永不触发的潜在 bug**。⬜ 待:Power Treads 三属性切换(需物品系统支持逐实例可变属性,较重)、Bloodstone 充能/死亡爆炸/法术吸命、Silver Edge Break、Diffusal 充能限制。

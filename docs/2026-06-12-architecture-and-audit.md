...
## 6. 加固进度(增补,随 Wave 推进刷新)

- **A 级(对局正确性)**:A1/A2/A3/A4 全部修复(Wave 1)。
- **B 级(确定性/架构)**:B2(目标校验下沉 sim)、B3(种子外显)、B4(去墙钟)完成;
  B1(人类输入命令队列)推迟为独立里程碑(单机无症状,挡 replay/lockstep)。
- **C 级(测试护栏)**:C1 端到端确定性、C2 sim 纯净性 已补齐。
- **D 级(打磨)**:D1(holder 泄漏)、D2(昼夜/视野时序)、D3(boss 特效坐标)、
  D4/D5(渲染热路径分配)、D8(英雄撞色)、D11(野怪回营重置)、D7(死 pendingItemSlot)、
  D10(死 rax teamGold)**已修复并补回归测试**;D6(预览圈真实半径)、
  D9 余下(孤儿组件/未具名死常量)推迟为独立任务。
- 当前测试规模:**63+ 文件 / 630+ 用例全绿**,typecheck 与生产构建通过。

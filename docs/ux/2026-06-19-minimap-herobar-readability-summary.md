...
### 小地图守卫图标
- 守卫系统**早已实现**(`ward_obs` 视域守卫 + 岗哨守卫真视,接入视野系统,items.test 已覆盖),但小地图只画成通用小点。改为**眼形图标**(视域守卫金 `#ffd54f` / 岗哨真视青 `#5fd0d0`,圆 + 暗瞳),追踪视野/反隐覆盖。迷雾门控天然正确(自己守卫可见、敌方守卫需岗哨揭示,沿用单位循环的 `isVisibleTo`)。

## 后续(留档)

- Alt 同时显敌方英雄装备/技能冷却(需敌方道具可见性规则)。
- 中立物品系统 / 战斗日志面板(用户保留为"大方向",待其指定)。

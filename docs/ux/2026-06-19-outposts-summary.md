...
- **收益**:占领方此后每 `OUTPOST_XP_INTERVAL`(7min)获得一次**团队经验**,数额随游戏时长递增(`outpostXp(t) = 120 + floor(t/60)*6`)。
...
- 测试:`tests/outposts.test.ts`(6:初始化/经验递增/单方占领/僵持/再占/周期经验)。
...
- 真实输入浏览器验证:右键移动英雄至前哨 → 站位 6s → 归属翻转 + 小地图 ◆ 变队色(见下方结果)。
- **batchsim 平衡验证**:**8/8 种子全部 decisive + sane**(断言含阵营胜率 ≤80%、时长 15-75min)→ 前哨团队经验对称、未破坏平衡。

## 后续(留档,需 GPT 或你)

- **3D 可见结构**:前哨在 3D 世界的实体造型(旗帜/方尖碑)属 GPT 视觉轨。

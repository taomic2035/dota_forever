...
- HUD XP progress first pass is implemented:
...
- Scoreboard net worth first pass is implemented:
...
- Shop recipe progress first pass is implemented:
...
- Shop keyboard-buy first pass is implemented:
  ...
  - recipe rows now show the next missing component and Shift+Enter buys that component when it is currently buyable.
  - Ctrl+Enter now buys the current visible recipe row's currently buyable missing components in one pass.
- Shop stash retrieval first pass is implemented:
  - pure model: `src/ui/shopStashActionModel.ts`
...
- Courier minimap marker first pass is implemented:
  - pure model: `src/render/minimapCourierMarker.ts`
  - focused tests: `tests/minimapCourierMarker.test.ts`
  - minimap integration: live couriers render as distinct diamond markers with `ally`, `busy`, `danger`, or `enemy` tone.
  - allied courier marker stays visible to the owning viewer; enemy courier marker remains vision-gated.
- Courier low-health HUD danger feedback is implemented:
  - live couriers at or below 35% HP preserve their current task status but use danger tone.
  - low-health detail is prefixed with `Low HP / ...`.
  - low-health action label becomes `F2 select / save courier`.
- Courier death announcement first pass is implemented:
  - pure model: `src/ui/announceModel.ts`
  - focused tests: `tests/announceModel.test.ts`
  - allied courier death uses red central warning text and alert audio cue.
  - enemy courier death uses gold central positive-event text and announce audio cue.
- Courier death location pulse first pass is implemented:
  - pure model: `src/ui/courierEventFeedback.ts`
...
- Automatic danger ping first pass is implemented:
  - allied courier death now uses `dangerPing`; enemy courier death remains regular `ping`.
  - pure building alert model: `src/ui/buildingAttackAlertModel.ts`
  - focused tests: `tests/buildingAttackAlertModel.test.ts`
  - allied buildings attacked by enemy heroes now use `dangerPing` while preserving the existing per-building cooldown and alert audio.

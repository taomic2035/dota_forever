...
function think(w: World, u: Unit, st: BotState): void {
  ...
  // 注:曾尝试「主动 gank 游走」(健康 bot 离开兵线去猎杀 1100 外孤立弱敌),batchsim 复验否决——
  // bot 离线游走 → 少推塔少 farm → 节奏拖长,一局 90min 不决(决胜率 7/8)。bot AI 的时间分配是为
  // 「决胜节奏」精调的,改变时间分配(游走/续航)的增强都会破坏决胜闸门;强化既有战斗(攻击物品/BKB)
  // 才安全。故不引入主动 gank。

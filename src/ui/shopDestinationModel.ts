...
export type ShopAccess = 'home' | 'secret' | null;
...
export function buildShopDestinationModel(input: {
  ...
  const allowed = item.secretShop ? effectiveShop === 'secret' : effectiveShop === 'home';

  if (!allowed) {
    return blocked(item.secretShop ? 'Secret' : 'Shop', item.secretShop ? 'Need secret shop' : 'Need home shop');
  }
  ...
  if (hero.inventoryFreeSlots > 0) {
    return destination('hero', 'Hero', 'Goes to inventory', 'ready');
  }

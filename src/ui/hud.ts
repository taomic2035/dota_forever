...
export class Hud {
  ...
  /** 技能程序化图标:按主导标签 + 家族色绘制 SVG 符号(无需逐技美术,提升图标语义)。 */
  private abilityIcon(def: AbilityDef): string {
    ...
    return `<svg viewBox="0 0 24 24" width="30" height="30" style="display:block">${inner}</svg>`;
  }
  ...
  private abilitySlot(world: World, hero: Unit, i: number, ux?: UxFeedback): string {
    ...
    return `<div title="${def.name}${def.ultimate ? ' (Ultimate)' : ''}: ${def.description}${aghsDesc}"
      ...
      <div style="opacity:${lvl > 0 || learnable ? 1 : 0.45}">${this.abilityIcon(def)}</div>

...
- **特效层**(`render/fx.ts` + `fxStyle.ts`):消费每 tick 事件流,把约 **280 种技能/物品特效**按载荷映射为几何原型——`pos2`→光束(闪电折线)、`radius+duration`→持续力场、`radius`→扩张环、点状→爆发/上升/下沉/闪现/裂地;按名称关键词推断火/霜/雷/影/毒/圣/奥术配色。含**浮动战斗文字**(伤害数字、击杀赏金、连杀播报)、升级金环、建筑爆炸、英雄死亡迸碎。
- **建筑美术**:防御塔(锥形塔身+城垛+发光核心+分级点)、近战/远程兵营(双坡顶+兵种标识)、主基地(六边形多环要塞+无敌护盾环)、泉水(发光圣池)。
- **地形**:更高分辨率烘焙、树冠聚簇带投影、高台悬崖底缘阴影、河道水面高光、符文点地面标记。

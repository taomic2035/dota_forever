# DOTA FOREVER — 经典 DotA 1 玩法复刻

一个在浏览器里运行的经典 5v5 三路推塔对战游戏:正补反补、战争迷雾、昼夜交替、
神符野区、河道 Boss、装备合成、买活回城——完整复刻 DotA 1 时代的核心玩法体验。

**TypeScript · 2D(Canvas2D)与可选真 3D(Three.js)双渲染器 · 1700+ 项单元/仿真测试 · AI 整局对战可分出胜负。**

> **v2.2** · 在 2.1 操控体系上深化:**单位碰撞 / body-block**、暴击伪随机(PRD)、助攻金(赏金切出式)/ 幻象死亡金 / Roshan 奶酪等**经济保真**;**操控 UX 大整合**(物流面板 / 信使指令 / 沟通层 / buff 图标+控制条 / 威胁方向 / 商店 V2 / 小地图通信)+ **地图机制可读性**(高地/野怪/神符/林影看得见)。模拟确定性不变、渲染与模拟严格分离;**1764 测试全绿、batchsim 8/8 决胜**。
>
> **v2.0** · 单机 vs AI 完整成品 · 新增**真 3D 渲染层**(`?renderer=3d`):全单位精细模型 + 行走/攻击/施法/死亡动作、实时阴影、昼夜光照 · 核心机制经三轮对抗性审计对齐经典 DotA 1。默认仍为稳定的 2D 渲染。

> **内容边界声明**:本项目复刻的是**游戏机制**(规则与数值模型不受版权保护),
> 不包含任何受版权保护的内容——全部英雄、技能、物品的名称与文案均为原创,
> 美术为程序化绘制,音效为 WebAudio 合成,不使用任何暴雪/Valve 素材。

![选人界面](docs/screenshots/menu-pick.png)

| 英雄造型(每英雄独立配色 + 图腾纹章 + 武器) | 兵线交战 + 技能/战斗特效 |
|---|---|
| ![英雄造型](docs/screenshots/art_showcase.png) | ![兵线交战](docs/screenshots/art_lane.png) |
| 基地建筑(分级塔 / 兵营 / 主基地护盾 / 泉水) | 团战:浮动伤害数字 + 法术特效 |
| ![基地建筑](docs/screenshots/art_base.png) | ![团战](docs/screenshots/art_teamfight.png) |

## 快速开始

```bash
npm install
npm run dev        # http://localhost:5180
```

- 主菜单选择英雄开始 5v5(你 + 9 个 AI),或观战纯 AI 对局。
- URL 参数:`?mode=play&hero=zola&seed=42&speed=1` / `?mode=spectate&speed=8`
- **真 3D 渲染**(可选):任意对局加 `&renderer=3d`,如 `?mode=spectate&renderer=3d`。
- **3D 素材陈列**:`?mode=hero3d-preview`(10 经典英雄)/ `?mode=resource3d-preview`(兵/野怪/建筑/物品/特效/地图道具)。

```bash
npm test           # 全部单元与仿真测试(含一整局 AI 对战回归)
npm run batchsim   # 多种子整局平衡批跑(约 5 分钟)
npm run build      # 生产构建(tsc + vite)
```

## 真 3D 渲染层(2.0,可选)

2.0 在不改动模拟的前提下新增了一套 **Three.js 真 3D 渲染器**(`src/render3d/`),与 2D 渲染器并存。任意对局 URL 加 `&renderer=3d` 启用;默认仍是经过整局验证的 2D 渲染器,保证稳定。

![hero3d 精细英雄素材预览](docs/screenshots/ux-hero3d-preview.png)

- **全单位 3D 模型 + 动作**:行走 / 待机 / 攻击 / 施法 / 死亡。
  - **英雄**:10 名经典英雄使用精细 PBR 模型(部件化建模 + 生成式贴图 albedo/normal/orm/emissive + 描边 + 辉光壳 + 动作 clip);其余 102 名由参数化人形程序化生成。
  - **兵 / 野怪 / Boss / 建筑**:精细素材(队色配色、选取环、投影);守卫程序化兜底。
- **场景**:俯视透视相机、定向光实时阴影 + 半球补光、昼夜光照过渡、森林地形(`InstancedMesh`)、3D 特效(技能光束 / AoE 环 / 弹道 / 受击火花)、头顶血条蓝条叠加层。
- **模型层可插拔**:`renderer3d` 按 `英雄精细素材 → 单位精细素材 → 程序化兜底` 三级优先分派;素材契约可平滑替换为未来的 GLB 资产。
- 渲染与模拟严格分离:`render3d/` 只读 `World` 状态,`sim/` 零改动。
- 大规模团战的性能打磨为进行中的后续工作(见 CHANGELOG)。

## 操作(忠实经典)

| 按键 | 功能 |
|---|---|
| 右键 | 移动 / 攻击目标(对选中的全部可控单位下令) |
| A + 左键 | 攻击移动 / 强制攻击(可反补血量<50%的己方小兵) |
| Q W E R | 技能(普通/快速/智能施法可设置;Alt = 自施) |
| 点击技能栏 **+** | 升级技能(升级/属性加点,有技能点时亮起金色) |
| 1–6 | 使用物品 · T | 回城卷轴(专属槽) |
| **Shift + 指令** | 队列命令(连续执行) |
| 左键 / Shift+左键 / 拖拽框选 | 选中单位 / 增减选 / 框选己方可控单位 |
| F1 / F2 / F3 | 选英雄 / 选信使 / 全选可控单位 |
| Ctrl+数字 绑定,数字 召回 | 控制组(可在设置切换「数字行=物品/控制组」) |
| F | 商店面板 · B | 买活 · G | 守护符(强化己方建筑) |
| S / H | 停止 / 保持原地 |
| 空格 | 镜头回到英雄 · Tab | 记分板 · Alt(按住) | 显示防御塔攻击范围 |
| Alt+左键 / Alt+点小地图 | 发信号(ping) |
| P / ESC | 暂停 / 菜单 · 改键 | 暂停菜单内完整自定义热键 |

## 已实现的经典机制

**战斗模型(WC3 风格)**
- 攻击类型 × 护甲类型克制矩阵(英雄/普通/穿刺/攻城/魔法 × 六种护甲)
- 护甲减伤公式 `0.06A/(1+0.06A)`、负甲增伤、英雄 25% 基础魔抗
- 三属性体系:力量(19 血/点)、敏捷(护甲+攻速)、智力(13 蓝/点),主属性加攻
- 攻击前摇/后摇、弹道飞行、闪避、暴击、吸血、纯粹伤害
- **低打高 25% 落空、低地看不见高地**(河道视野劣势是真实的)

**经济与成长**
- 正补得金、**反补**(敌方 0 金 + 50% 经验)、经验共享圈(1300)
- 击杀赏金随等级与连杀增长、死亡掉金、买活(等级² 定价)
- 等级 1–25,技能 4/4/4/3,大招 6/11/16,黄点属性加成 ×10

**地图与节奏**
- 15040² 世界、三路对称地图、河道、双方野区、高台基地(坡道入口)
- 兵线 90 秒首波、30 秒一波、每 7 波投石车、每 7.5 分钟成长
- 破兵营出超级兵、六营全破出精英兵;基地双塔不破主基地无敌
- 战争迷雾(已探索灰雾)、昼夜 5 分钟交替(夜晚视野骤减)
- 神符每 2 分钟(加速/双倍/恢复/隐身)、魔法药瓶可装符
- 双方野区 4+1 营地整分钟刷新、远古营地高魔抗
- **深渊领主**(河道 Boss):随时间成长,掉落**不灭之盾**(原地复活一次),8–11 分钟重生
- 防御塔仇恨规则(优先小兵、保护被打的英雄)、泉水高伤防御与回复光环

**物品系统(140+ 件,经典完整)**
- 基础散件 / 秘密商店高级件 / 配方卷轴自动合成(支持连锁合成,价格=部件和+卷轴)
- 主动控制:闪烁短刃(受伤锁 3 秒)、原力法杖(推动)、挽紧之杖(缠绕)、纷争面纱(沉默)、勇气勋章(破甲)、魔法免疫
- 硬控/团战主动:**妖术法球**(变形)、**飓风之杖**(旋风)、**深渊之刃**(无视魔免眩晕)、**虚空面纱**(削魔抗)、**洞察烟斗**(团队护盾)、**刷新之球**(重置冷却)、幽魂权杖(虚化)
- 法球特效:风暴之锤/雷神之锤(连锁闪电)、湮灭之刃(破甲)、寒霜之心(冰冷减速)、净魂之刃(燃魔)、辉光(灼烧)、吸血/暴击
- 核心终极:蝶舞之翼(闪避)、圣剑(+330 攻击)、先锋之盾(减伤)、神灭之球(法术爆发)、金箍棒/缚足锤(重击眩晕)
- 团队增益:梅肯斯姆(团队治疗)、远古战鼓(团队加速·充能)、莲花宝珠(护盾自净)
- 合成线:萨格之刃→赤红甲→天堂之戟(致残/缴械)、幻影斧(分身自净)、点金手(点化发育)、天鹰之戟→飓风长戟(攻击距离/推开)、法术之刃→以太之镜→奥术核心(法术增强)
- 升级控制:雷神枷锁(范围缠绕+连锁)、血棘(沉默+暴击)、银月之冠(破甲/强化)、静默之刃(隐身突袭)
- 团队装:深红甲盾(团队减伤)、卫士胫甲(团队治疗+驱散)、月之碎片(极致攻速)、阴邪之罐(灼伤/治疗)、支配头盔(吸血光环)
- 进阶主动:以太之刃(虚化)、流星锤(延迟范围晕)、否决坠饰(驱散禁锢)、永恒之纱(法术屏障)、渔叉(拉近自身)、风之波荡(自旋无敌)、散华/永世法衣
- 位移与续航:远行之靴(全图传送)、相位鞋(穿行)、影锋/微光/诡计之雾(隐身)、血石、静谧之鞋
- 保命/辅助:永恒之碟(减伤保命)、神圣链锭(爆发治疗)、阴邪契约(削抗破甲)、真知宝石(持有真视)、亡者之牙(虚灵攻击)
- 消耗品散件:仙灵之火(回血)、魔法芒果(回蓝)、王冠(全属性)、隼之锋刃
- 召唤物品:亡者之书(召唤随从);充能物品:魔棒(敌方施法积充能)、萃取之瓶
- 视域守卫(插眼)/ 岗哨守卫(真眼)/ 显影之尘 — 完整视野博弈
- 储藏处、出售半价返还、死亡购买入库

**英雄(112 名全原创,机制致敬经典原型,经典完整阵容规模)**

每名英雄都有 3 个普通技能 + 1 个大招(QWER),含独立 AI 释放评分。支持的英雄机制涵盖:
先手开团 / 物理核心 / 法系爆发 / 治疗护盾 / 召唤(树人·守卫·亡魂)/ **幻象**(分身出伤打折受伤翻倍)/
嘲讽 / 反伤 / 换位 / 处决 / 流形变身 / 承伤减免 / 法术增强 / 强制黑夜 /
**多重施法**(技能概率连发)/ **劈砍强化**(武器溅射 buff)/ **巨型团控**(拉拽聚怪开团)/
**寒霜连锁**(弹道弹跳递增)/ **吞噬成长** / **末日**(沉默+缴械+灼烧)/ 海妖外壳(伤害减免+自净)/
**法力虚空**(按缺蓝爆发)/ **决斗**(1v1 锁定·胜者永久成长)/ **混沌幻象** / 齿轮钩索 /
**黑洞**(引导吸入+眩晕)/ 守护天使(团队物理免疫)/ 退化光环 /
**薄葬**(免死保命)/ **延迟激流** / 潮位标记(拉回原点)/ 技能箭矢(缚击/强力击)/
相位免疫 / 月影团队隐身 / 神之力量(攻击爆发)等。

部分代表英雄:

| 英雄 | 定位 | 招牌 |
|---|---|---|
| 雷恩·铁壁卫士 | 力量先手 | 雷霆战锤(投掷眩晕)/ 泰坦之力 |
| 格罗什·钩魂者 | 力量游走 | **锁链魂钩(把人拖回来)** / 碎骨肢解 |
| 佐拉·雷霆先知 | 智力爆发 | 连环闪电 / 静电场 / **雷神之怒(全图)** |
| 奥兰·晨光牧师 | 智力治疗 | 圣疗术 / 信仰守护(护盾)/ 神圣庇护(物免) |
| 邓肯·重装壁垒 | 力量坦克 | 战吼嘲讽 / 壁垒领域 |
| 萨娅·幻影舞者 | 敏捷幻象 | 虚影分身 / 残影闪 / 幻象大军 |
| 凯隆·神射手 | 敏捷核心 | 穿透射击 / **狙杀(超远程)** |
| 奈雅·自然先知 | 智力召唤 | 召唤树人 / 全图自然之怒 |
| 收割者·格里姆 | 力量消耗 | 缺血斩 / **死神收割(处决)** |
| 编织者·薇芙 | 敏捷游走 | 缩地隐身 / 双重攻击 / 时光倒流 |
| 玛格·巨角战兽 | 力量先手 | 穿刺冲撞 / 强化(劈砍)/ **两极反转(拉拽全场)** |
| 杜姆·噩梦领主 | 力量游走 | 吞噬成长 / 焦土 / **末日(沉默缴械灼烧)** |
| 恩尼·虚空使者 | 智力召唤 | 噩咒 / 亡魂召唤 / **黑洞(引导吸入团控)** |
| 里昂·决斗将军 | 力量决斗 | 强压 / 鼓舞冲锋 / **决斗(1v1 锁定·胜者永久成长)** |
| 戴泽·微光萨满 | 智力守护 | 剧毒之触 / **薄葬(免死)** / 暗影波 / 编织 |
| 斯文·流放骑士 | 力量核心 | 风暴之锤 / 战吼 / 巨力挥砍 / **神之力量** |
| 默罗·殁世巫师 | 智力法核 | 奥术天球 / **星体禁锢(放逐)** / 精神虚妄 |
| 班恩·噩梦祭司 | 智力控制 | 弱化 / 吸脑 / 噩梦(沉睡)/ **末日缠绕(引导)** |
| 提尼·巨岩 | 力量先手 | 山崩 / **投掷(抓投单位)** / 长大 / 巨化狂怒 |
| 灵狲·齐天灵猴 | 敏捷核心 | 棒击大地 / **金箍戏法(连击强化)** / 灵敏 / 大圣天兵 |
| 奥锐·神谕者 | 智力守护 | 命运止境 / 命运敕令 / 净化之焰 / **虚妄之诺(救人)** |

> 完整 112 名见 `src/data/heroes/`(`index.ts` + `batch2~19.ts`),游戏内选人界面可查看全部技能 tooltip;已达经典 DotA 1 完整阵容规模。

**AI(9 个 bot)**
- 三层决策:战略(分路/撤退/防守/集结)→ 战术(技能评分释放/目标选择)→ 微操(补刀反补窗口/推塔)
- 按定位出装(含合成链)、TP 防守基地、塔下危险规避
- 后期收敛:40 分钟后抱团推最薄弱一路 → 对局必然分出胜负(多种子批跑验证 43–54 分钟决胜)

## 画面与表现(全程序化矢量绘制)

渲染层只读模拟状态与事件流,模拟核心绝不触碰 DOM。所有造型与特效均为 Canvas2D 程序生成,无任何位图素材。

- **单位建模**(`render/unitArt.ts`):由单位属性推导矢量造型——力量/坦克魁梧、敏捷带刃、智力法袍、野怪兽形带刺、小兵 grunt/archer、攻城器械带轮。**每名英雄以 `heroDef.color` 上色并叠加图腾纹章**,112 个英雄各具辨识度(此前所有单位都是同色圆圈)。
- **动画状态**:全部由模拟字段推导(不改 sim)——行走起伏、攻击前摇后摆(`windupUntil`)、施法蓄光、引导脉动、受击闪白、眩晕转星、魔免护环、隐身半透。
- **状态条**:英雄血条上方一排限时 buff/debuff 方块(红=控制、橙=敌方减益、绿=增益),底部细条示剩余时间——一眼看清谁被眩晕/减速/有增益。
- **特效层**(`render/fx.ts` + `fxStyle.ts`):消费每 tick 事件流,把约 **280 种技能/物品特效**按载荷映射为几何原型——`pos2`→光束(闪电折线)、`radius+duration`→持续力场、`radius`→扩张环、点状→爆发/上升/下沉/闪现/裂地;按名称关键词推断火/霜/雷/影/毒/圣/奥术配色。含**浮动战斗文字**(伤害数字、补刀 +金币/拒绝、击杀赏金、连杀播报)、升级金环、建筑爆炸、英雄死亡迸碎。
- **建筑美术**:防御塔(锥形塔身+城垛+发光核心+分级点)、近战/远程兵营(双坡顶+兵种标识)、主基地(六边形多环要塞+无敌护盾环)、泉水(发光圣池)。
- **地形与地标**:更高分辨率烘焙、树冠聚簇带投影、高台悬崖底缘阴影、河道水面高光;符文点 / 本方商店范围环 / 秘密商店 ◈ / 深渊领主巢穴 ☠ 标记(小地图常驻、主视图随视野隐现)。
- **弹道**:普攻定向箭矢(随阵营着色)、技能发光法球带拖尾。
- **HUD/反馈**:顶栏团队击杀比分;英雄面板技能栏(Q/W/E/R 等级点 / 冷却 / 蓝耗 / 点击 + 加点);血/蓝条带数值;限时状态条;小地图英雄按本色显示 + 地标。

## 架构

```
src/
  core/     vec2 · 种子随机(mulberry32) · 数学工具
  data/     balance.ts(全部数值单一真相源) · heroes/ · items · neutrals · mapLayout
  sim/      确定性 30Hz 定步模拟(不触 DOM):
            world(tick) → modifiers(万物皆 buff) → 兵线/AI → 指令/战斗 → 弹道 → 建筑 → 经济 → 视野
  render/   Canvas2D:地形烘焙 / 程序化单位造型(unitArt)/ 特效层(fx·fxStyle)/ 迷雾 / 小地图
  ui/       DOM HUD:面板 / 商店 / 记分板 / 击杀播报 / 结算 / 菜单
  audio/    WebAudio 程序化合成音效
tests/      1700+ 项:战斗矩阵/经济/寻路/视野/技能/合成/野区/Boss/英雄技能/物品/美术描述符/特效/选择操控/预览形状/确定性/整局仿真
```

关键设计:
- **Modifier 系统**承载一切状态(眩晕/减速/光环/DoT/护盾/隐身),技能=数据+行为函数
- **Order 指令系统**:人类输入与 AI 输出同构,天然支持回放与未来 lockstep 联机
- 模拟确定性:同种子同输入 → 逐 tick 一致(测试可精确断言)

## 有意的简化(与原作的差异)

- 数值为"经典近似"而非逐补丁精确(全部集中在 `src/data/balance.ts`,易调)
- 信使为自动投送(储藏物品自动配送),可被选中下达移动/攻击;经典的全程手动驾驶取送为简化
- 神符走近自动拾取;有空药瓶时自动装瓶
- 树木不可砍伐、无录像回放系统(幻象与召唤物均已实现)
- 联机未实现(本作定位单机 vs AI;架构已预留确定性 lockstep 接口)
- (注:单位碰撞 / body-block 已于 2.2 实现;树木仍为不可穿越阻挡、无树木 LOS 遮断,后者待 bot 收尾 AI 配套)

## 开发

数值调整只改 `src/data/balance.ts`;新英雄=新增 `src/data/heroes/*.ts` 数据文件;
新物品=追加 `src/data/items.ts` 条目(配方卷轴自动注册)。

```bash
npm run typecheck   # 严格 TS
npm test            # vitest
node scripts/shot.mjs <url> <out.png> [waitMs] [evalExpr]   # 自动化截图/冒烟
```

## Documentation And UX Handoff

- [Documentation index](docs/README.md)
- [UX design and summary index](docs/ux/README.md)
- [Screenshot evidence index](docs/screenshots/README.md)
- [Source map](docs/source-map.md)
- [Implementation plans](docs/superpowers/plans/)
